(ns openvino.main
  "Kotodama WASM entrypoint for the Openvino clean-room actor (L4) — Clojure port.
  ...
  facts (`openvino.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "openvino")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"openvino.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "openvino-compat" :tier tier :entities entities} 200])
...
(defn start! [] :openvino-compat/ready)
